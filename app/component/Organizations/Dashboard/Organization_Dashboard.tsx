...
export default function Organization_Dashboard({ organization_details }: { organization_details: Public_Facing_Info }) {
  ...
  const handleEdit = async (userId: string, updatedLevel: number) => {
    ...
    // Get current authenticated user (the owner)
    const { data: itself, error: itselfError } = await supabase.auth.getUser();
    if (itselfError) throw itselfError;
    const self_id = itself.user.id;

    // Build the users array for the database.
    // Ensure the current user (owner) always has level 4.
    ...
    // ❌ DO NOT push any extra entry – that caused the bug
    // The owner is already included in updatedMembers with level forced to 4.
    ...
    if (updateError) {
      ...
      await fetchOrganizationAndMembers(); // revert to server state
    } else {
      setActionMessage({ type: "success", text: "User level updated." });
      await fetchOrganizationAndMembers(); // refresh with latest data
    }
  ...
  const handleKick = async (userId: string) => {
    ...
    // Remove organisation from user's metadata
    ...
    // Update organisation's users list
